...
public class ProductService {
	...
	public Map<String, Object> getProductList(ProductForm productForm) {
		...
		try {
			...
			// 목록 Total Count 조회
			int totalCount = productMapper.selectProductTotalCount(paramsMap); // 작성
			resultMap.put("TOTAL_COUNT", totalCount);
			
			// 목록 조회
			List<Map<String, Object>> product_list = productMapper.selectProductList(paramsMap);// 작성 
			...
			int lastPageNum = (int) Math.ceil((double) totalCount/pageSize); // (totalCount < pageSize)?1 : (int)(totalCount/pageSize);

			// 페이지 블록 범위 계산
			int startBlockPage = 1;
			int endBlockPage = lastPageNum; // 작성
